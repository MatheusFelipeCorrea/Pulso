...
- **Duas ressalvas concretas, não refutações:**
  1. O `collectCoverageFrom` do `jest.config.js` **exclui explicitamente** `insightService.js`, `googleCalendarService.js` e `googleCalendarSyncService.js` do cálculo de cobertura — todos com lógica de negócio real e não trivial. O "~95%" é medido sobre um subconjunto que já deixa de fora essas peças.
