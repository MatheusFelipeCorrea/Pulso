# Documentação do Projeto Pulso

Índice da documentação de produto, requisitos e engenharia.

## Produto e planejamento

| Documento | Conteúdo |
|-----------|----------|
| [Requisitos/Readme.md](./Requisitos/Readme.md) | RFs/RNFs e progresso por módulo |
| [Regras de Negocio/RegrasDeNegocio.md](./Regras%20de%20Negocio/RegrasDeNegocio.md) | Regras de negócio (modos de uso, VT, benefícios) |
| [Roadmap/Roadmap.md](./Roadmap/Roadmap.md) | Fases de implementação |
| [Analise-Produto.md](./Analise-Produto.md) | Gaps, oportunidades e prioridades sugeridas |
| [Modulos/Grupos.md](./Modulos/Grupos.md) | Módulo Grupos — API, UI e gaps de usabilidade |

## Engenharia

| Documento | Conteúdo |
|-----------|----------|
| [Deploy/Hospedagem.md](./Deploy/Hospedagem.md) | Vercel, Neon, cron, variáveis |
| [Diagramas/Readme.md](./Diagramas/Readme.md) | UML e diagramas de arquitetura |
| [../Codigo/Pulso/api/Documents/](../Codigo/Pulso/api/Documents/) | API, banco, rotas |
| [../Codigo/Pulso/web/Documents/](../Codigo/Pulso/web/Documents/) | Frontend, rotas, DS |

## Protótipos

| Documento | Conteúdo |
|-----------|----------|
| [Prototipos/Readme.md](./Prototipos/Readme.md) | Referências visuais |

## Epics (GitHub)

Cards de implementação em [`.github/plans/cards/`](../.github/plans/cards/).
