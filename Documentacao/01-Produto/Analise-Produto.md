...
- **Core financeiro:** auth (email + Google, cookies httpOnly), transações com recorrência, categorias com **`grupoBeneficio`** (VA/VR; recurso VT legado rejeitado), tags, orçamento com alertas e flag `orcamentoExcedeRenda`.
...
- **Importação:** OFX/CSV/XLSX/PDF (Gemini) — upload → preview → confirmar no modal do dashboard (RF-134–137, RF-139). Falta **RF-138** (aprendizado).
...
- **Planos Free/Premium:** gate no backend (`requirePremium` em grupos, viagens e sync Google Calendar); demo sem billing.
...
`modoUso`, renda fixa, VA/VR e preferências existem no banco mas **não têm tela** dedicada (`/settings` / `/profile`).

**Sugestão:** `/settings` (RF-068–079).

### 2. IA prometida, parcial

Landing honesta com badges; backend: regra simples (maior gasto). **Gemini** já usado na importação PDF; chatbot / insights / RF-015 (quick-add) ainda sem UI completa.

### 3. Divisão de despesas ✅

Módulo **implementado** (RF-106–111). Falta **integração** com toggle RF-091 no detalhe do grupo.

### 4. Grupos — gaps restantes

| Gap | Prioridade |
|-----|------------|
| Integrar RF-091 ↔ `/expense-split` | Média |
...
| Gap | Prioridade |
|-----|------------|
| RF-138 aprendizado com ajustes do usuário | Média |
| Onboarding RF-130–133 (inclui rota import vs saldos manuais) | Alta quando for prototipar |
...
| 2 | RF-015 quick-add no dashboard | Fecha o gap do dashboard (8/9) |
| 3 | RF-138 aprendizado na importação | Fecha o módulo de import |
| 4 | Gemini insights MVP | Diferencial landing |
| 5 | Grupos ↔ expense-split | RF-091 completo |
